...
csp_document <- function(toc = FALSE,
  ...
  Sys.setenv(TEXINPUTS = system.file(package = "CSP"))
...
Recall when we used `Sys.setenv` in our `csp_document` function. Since we set the `CSP` R package as a directory for LaTeX inputs. We create the `images\` directory in the `inst\` directory to place any logos that we might want with our branded template.
